...
## 10.1 Cost boundary and major tag workflow

Do not start paid or usage-metered workflows automatically. In particular,
multi-agent review/research workflows must not run unless the user explicitly
asks for that run in the current session.

For major tag or release work (`v1.0.0`, `v2.0.0`, or another project release
tag that changes the supported platform contract), use only the repository's
normal local/CI checks by default. If a multi-agent review would be useful,
recommend it and wait for explicit user approval before starting it.
...
- Run the normal validation gates that apply to the touched scope, such as
  `make lint`, `make test`, `make kube-lint`, and the existing GitHub Actions CI.
