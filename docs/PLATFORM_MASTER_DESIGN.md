...
작성일: 2026-05-04
갱신일: 2026-08-17
상태: **Repository Authority Mirror · NOT an independent canonical**
...
이 섹션은 authority revision `AR-2026-08-17.1`의 repository mirror다. 이 저장소가 cross-repo meaning을 독립적으로 소유한다는 뜻이 아니며, 상위 authority와 충돌하면 task를 중단하고 mirror revision을 갱신한다.
...
### 4.10 관측하지 않은 것을 기록하지 않는다 — 위키 소유 (포인터)

§4.1–4.9와 달리 이 불변식의 정본은 이 문서가 아니라 **Platform Spec Wiki §1.10
「관측하지 않은 것을 기록하지 않는다」** (`개인 홈 › Platform Spec Wiki › 1. constitution`
§1.10, <https://app.notion.com/p/3ae86e6dd1d1811c8bfad78e5f878f60>) 이다. 관측 결과를 담는 필드·지표에 리터럴을 직접
넣지 않는다는 규칙이며, 「`IMPLEMENTED` 기준(required-check 백킹)」·「SoT 구분」과 한 묶음인
위키·헌법 체계 거버넌스 규칙이라 플랫폼 런타임 설계인 §4의 성격과 다르다.

전문을 여기로 옮기지 않는다 — 크로스레포 자산을 컴포넌트 저장소(이 문서는 NodeVault
저장소 문서다) 안에 복제하면 D-13(§6.9.2)을 새 자리에 재생산한다. **정본은 위키 §1.10이
소유하고, §4는 이 포인터만 둔다** (2026-08-04 결정, 위키 §1.10 「소유권 확정」 절 근거).
